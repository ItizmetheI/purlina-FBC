...
const startIndex = content.indexOf('function HeroCenterpiece({ quality }: { quality: \'high\' | \'low\' }) {');
const endIndexStr = 'function LightShafts() {';
const nextFuncIndex = content.indexOf('function LightShafts() {');
const endIndex = content.indexOf('function ExplodingBlade()', startIndex);
let before = content.substring(0, startIndex);
let after = '';
if (content.indexOf('import ExplodingBlade', startIndex) !== -1) {
  after = content.substring(content.indexOf('import ExplodingBlade', startIndex));
} else {
  // Try finding default export
  after = content.substring(content.indexOf('export default function Scene', startIndex));
}

const newHero = `function HeroCenterpiece({ quality }: { quality: 'high' | 'low' }) {
...
fs.writeFileSync('src/canvas/Scene.tsx', before + newHero + after);
