...
const abstractFluidTarget = `function AbstractFluid({ quality }: { quality: 'high' | 'low' }) {
...
const abstractFluidReplace = `function AbstractFluid({ quality }: { quality: 'high' | 'low' }) {
  const count = quality === 'high' ? 120 : 60;
...
sceneContent = sceneContent.replace(abstractFluidTarget, abstractFluidReplace);

const abstractFluidLoopEndTarget = `      dummy.updateMatrix();
...
const abstractFluidLoopEndReplace = `      dummy.updateMatrix();
      meshRef.current!.setMatrixAt(i, dummy.matrix);
    });
    meshRef.current.instanceMatrix.needsUpdate = true;
...
sceneContent = sceneContent.replace(abstractFluidLoopEndTarget, abstractFluidLoopEndReplace);

// UPDATE document.hidden for Bubbles, DustMotes, LightShafts, HeroCenterpiece, ExplodingBlade, FluidSurface
...
// Wait, the replace for useFrame might insert it multiple times if not careful. Let's do it carefully.
